...
/**
 * Best-effort presence check for an `[edge_runtime.secrets]` (or
 * `[remotes.*.edge_runtime.secrets]`) section header — a raw regex over the
 * file text, deliberately NOT a config parse: `secrets set` no longer reads
 * config secrets (supabase/supabase#45242), this only powers the breadcrumb
 * messaging below so users who relied on the old merge learn where it went.
 */
const CONFIG_SECRETS_SECTION = /^\s*\[(?:remotes\.[^\]\n]+\.)?edge_runtime\.secrets\]/m;

const CONFIG_SECRETS_HINT =
  "[edge_runtime.secrets] in supabase/config.toml is not uploaded by secrets set — pass secrets explicitly or via --env-file.";
...
export const legacySecretsSet = Effect.fn("legacy.secrets.set")(function* (
  ...
  yield* Effect.gen(function* () {
    // Explicit inputs ONLY: `--env-file` entries first, then positional
    // `NAME=VALUE` pairs (which override the env file for the same name).
    //
    // `[edge_runtime.secrets]` from `supabase/config.toml` is deliberately NOT
    // read here — Go's `set.ListSecrets` mixes config secrets into the same
    // bulk-create payload, so `secrets set BAR=bar` also uploaded every config
    // secret and produced duplicate remote entries with differing digests
    // (supabase/supabase#45242). Config secrets stay a local-runtime concern
    // (`functions serve` sources them independently via the shared serve path);
    ...
    // Fail-open peek: an unreadable/absent config.toml just means no breadcrumb.
    const configDeclaresSecrets = CONFIG_SECRETS_SECTION.test(
      yield* fs
        .readFileString(path.join(runtimeInfo.cwd, "supabase", "config.toml"))
      ...
      return yield* Effect.fail(
        new LegacySecretsNoArgumentsError({
          message: `No arguments found. Use --env-file to read from a .env file.${
            configDeclaresSecrets ? `\n${CONFIG_SECRETS_HINT}` : ""
          }`,
        }),
      );
    }

    if (configDeclaresSecrets) {
      // The old behavior silently merged config secrets into this upload; exiting
      // 0 while uploading less than before deserves a signal (review finding on
      // supabase/supabase#45242 — a stderr note, not an error).
      yield* output.raw(`Note: ${CONFIG_SECRETS_HINT}\n`, "stderr");
    }
