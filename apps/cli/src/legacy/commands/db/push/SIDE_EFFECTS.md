...
| `SET LOCAL ROLE postgres` (first statement of every transaction db push opens: migration batches, roles.sql batches, history/seed table setup, vault upsert, seed-data transactions) | only when the remote session stepped down from a temp `cli_login_*`/`supabase_admin` login role; standalone pipeline-incompatible statements run unpinned (supabase/cli#6116) |
