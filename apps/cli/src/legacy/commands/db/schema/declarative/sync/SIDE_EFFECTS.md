...
| `docker`/`podman` container recreate for the local `db` (+ satellite stops before, restarts and Kong reload after) — the same primitives `db start`/`db reset` use, via `legacyResetLocalDatabase` — only on the failed-apply recovery path | TTY only, apply failed, and the user confirms "reset and reapply" |
