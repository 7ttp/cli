...
describe("legacy start integration", () => {
  ...
  describe("happy path", () => {
    ...
    it.live("completes when podman rejects creating a preserved volume (#6020)", () => {
      // `supabase stop` preserves volumes, so a stop→start cycle re-creates
      // existing ones — podman's CLI rejects that where docker's is idempotent.
      const route = defaultRoute();
      const { layer, analytics } = setup({
        route: (args) => {
