...
export interface LegacyContainerOpts {
  ...
  /**
   * `LegacyCliConfig.workdir` — the project's own working directory. Stamped onto every created
   * container as {@link LEGACY_CLI_WORKDIR_LABEL} so a later
   * `stop`/{@link legacyRollbackStart} can find this exact directory again.
   *
   * {@link LegacyStartContainerSpec.secretFiles} never use this path: they are archived in memory
   * and streamed into the container via `docker cp`. The label remains load-bearing for staging
   * under the
   * `<workdir>/supabase/.temp/start-secrets/<containerName>/` tree that this function
   * itself never writes, such as Edge Runtime's env-file and script artifacts. See
   * `shared/functions/serve.ts`'s `startEdgeRuntimeContainer` and
   * `legacy-start-secrets-cleanup.ts`.
   */
...
/**
 * `docker cp - <containerId>:/` — extracts an in-memory tar archive from stdin into the created
 * container. Streaming avoids any shared-filesystem assumption between this process and a
 * confined Docker/Podman client while retaining remote-daemon support.
 */
...
/**
 * Delivers every {@link LegacyStartContainerSpec.secretFiles} entry to the created, not-yet-started
 * container in one tar stream. Archive member names are container-root-relative, so extracting at
 * `/` writes each member to its exact `containerPath`. Bun emits regular archive members with mode
 * `0644`, matching the non-root readability of the files created by Go's entrypoint scripts.
 *
 * Using stdin is essential for confined clients such as the Docker snap: its private `/tmp` cannot
 * see a path created under the parent process's `/tmp`. It also avoids plaintext host files and
 * works over remote Docker contexts. Docker and Podman both document `cp -` as tar-from-stdin.
 */
